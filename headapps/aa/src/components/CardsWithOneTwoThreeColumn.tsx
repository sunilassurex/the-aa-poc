      ...
      <div className="bg-[#f6f6f6] md:py-[1.5rem]">
        <div className="wrapper mx-auto !px-[2.5rem] md:!px-[1.1rem] lg:!px-[2rem]">
          {contacts.map((item, index) => {
            if (item.variant === 'emergency') {
              return (
                <div key={index} className="grid">
                  <div className="flex flex-col items-center border-[8px] border-white bg-[#f6f6f6] px-[32px] py-[24px] gap-4 flex-[1_0_0] self-stretch shadow-[0px_0px_2px_0px_rgba(0,0,0,0.08),0px_1px_4px_0px_rgba(0,0,0,0.16)]">
                    {/* Row: Icon + Heading */}
                    <div className="flex items-baseline gap-[8px]">
                      <img
                        src="/warning-mark.png"
                        alt="Warning Icon"
                        className="w-[40px] h-[36px]"
                      />
                      <h2 className="!text-[24px] !leading-[32px] font-aa-sans !font-normal">
                        {item.title}
                      </h2>
                    </div>

                    {/* Column: Description + Phone */}
                    <div className="flex flex-col items-center gap-2">
                      <p className="text-[1.8rem] !leading-[26px] text-center !mb-[1rem]">
                        {item.description}
                      </p>
                      <p className="!text-[1.8rem] !leading-[26px] !font-bold !mb-0">
                        {item.phone}
                      </p>
                      <p></p>
                      <div className="block w-full md:hidden">
                        <a
                          href="tel:03330046046"
            ...
            return (
              <div key={index} className="md:max-w-[106rem] md:mx-auto md:px-[2rem] grid">
                <div
                  key={index}
